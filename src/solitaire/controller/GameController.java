...
public class GameController {
    ...
    private void onModeChange(String mode) {
        ...
        GameRecord currentRecord = currentMode.getRecord();
        ...
        if (currentRecord != null) {
            // Carry the in-flight recording across the switch so events
            // captured before the mode change aren't lost.
            currentMode.setRecord(currentRecord);
        } else if (recordingEnabled && currentMode.hasBoard()
    ...
    private void onRecordToggle(boolean enabled) {
        ...
        if (enabled) {
            ...
            // Persist whatever has been captured so far before clearing.
            GameRecord rec = currentMode.getRecord();
    ...
    private void onReplay() {
        // Persist any in-flight recording before swapping to replay mode.
        saveIfRecordingInProgress();
